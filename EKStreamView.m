...
const  float kHeaderView_X =0.0f; //origin.x of headerView 
const  float kHeaderView_Y =0.0f; //origin.y of headerView 
const  float kHeaderView_Width =320.0f;//width of headerView 
...
{
    if ([self.delegate respondsToSelector:@selector(streamView:didSelectedCellAtIndex:)])
    {
        [self.delegate streamView:self didSelectedCellAtIndex: ((EAStreamCell*)notification.object).index];
    }
}
...
- (void)streamCellDidSelected:(NSInteger)index
{
    if ([self.delegate respondsToSelector:@selector(streamView:didSelectedCellAtIndex:)])
    {
        [self.delegate streamView:self didSelectedCellAtIndex:index];
    }
}
...
- (void)layoutCellWithCellInfo:(EKStreamViewCellInfo *)info
{
    EAStreamCell<EKResusableCell> *cell = [delegate streamView:self cellAtIndex:info.index];
...
{
    if ([self.touchDelegate respondsToSelector:@selector(streamCellDidSelected:)]) {
        [self.touchDelegate streamCellDidSelected:self.index];
    }
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"CellSelected"
                                                        object:self
                                                      userInfo:[NSDictionary dictionaryWithObject:[NSNumber numberWithInt:self.index] forKey:@"IndexPath"]];
